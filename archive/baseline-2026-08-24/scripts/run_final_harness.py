...
def main(argv: Iterable[str]) -> int:
    ...
    wps_query = lookup.build_wps_query(as_dict={"line_class": normalized_line_class})
    ...
    wps_result = pwht_check.check_pwht_flag({"value": wps_docs}, normalized_line_class)

    if nde_docs:
        nde_result = material.check_material_ss([{"metadata": {"content": nde_docs[0].get("content", "")}}])
    else:
        nde_result = "No"
